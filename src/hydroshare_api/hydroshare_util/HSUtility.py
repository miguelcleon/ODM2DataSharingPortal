...
class HydroShareUtility(HydroShareUtilityBaseClass):
    ...
    def connect(self, scheme, username=None, password=None, token=None):
        if scheme == 'oauth':
            client_id = environ.get('HS_CLIENT_ID')
            client_secret = environ.get('HS_CLIENT_SECRET')
            if username and password:
                auth_util = HSUAuthFactory.factory(username=username, password=password, client_id=client_id,
                                                   client_secret=client_secret)
            else:
                auth_util = HSUAuthFactory.factory(client_id=client_id, client_secret=client_secret, token=token)
        elif scheme == 'basic':
            auth_util = HSUAuthFactory.factory(username=username, password=password)
        else:
            raise ValueError("'{scheme}' is not a valid authentication scheme. Must be 'basic' or 'oauth'.".format(scheme=scheme))

        self.hs = auth_util.authenticate()
    ...
    def purge_duplicate_gamut_file(self, resource_id, regex, conmmit_delete=False):
        ...
        for file_detail in duplicates_list:
            if conmmit_delete:
                self.hs.deleteResourceFile(resource_id, file_detail['name'])
