...
class UUIDAuthentication(authentication.BaseAuthentication):
    def authenticate(self, request):
        ...
        if not registration_queryset.exists():
            raise exceptions.AuthenticationFailed('teh')
        ...
        if str(registration.deployment_sampling_feature_uuid) != request.data[u'sampling_feature']:
            raise exceptions.AuthenticationFailed('blah')  # or other related exception
